...
int main(int argc, char** argv) {
    ...
    VideoCapture cap("/home/jeremymboe/cmpt130/belajarROS/deteksirobot/src/deteksirobot/src/video.mp4");
    ...
    const double staticBallX_cm = 8.975; // Static X position in cm
    const double staticBallY_cm = 1.125; // Static Y position in cm
    ...
    while (ros::ok()) {
        ...
        if (!contours.empty()) {
            ...
            if (largestContourIndex != -1) {
                ...
                robot_position.position.x = x_robot / cm_to_lapangan; // Convert cm to TurtleSim units
                robot_position.position.y = y_robot / cm_to_lapangan;
                robot_position.position.z = 0.0; // Assuming 2D plane
                robot_position.orientation.w = 1.0; // Default orientation
                ...
                ball_position.position.x = staticBallX_cm / cm_to_lapangan; // Static ball position in TurtleSim units
                ball_position.position.y = staticBallY_cm / cm_to_lapangan;
                ball_position.position.z = 0.0; // Assuming 2D plane
                ball_position.orientation.w = 1.0; // Default orientation
